  ...
  page.innerHTML = `
    <div class="page-header">
      ...
      <div class="filter-pills" id="buscarPills">
        ${['Todos', '16 Jun', '17 Jun', '18 Jun'].map(f =>
...
function buildPerfil() {
  ...
  page.innerHTML = `
    <div class="perfil-content">
      <div class="perfil-avatar">${icon('user', 40)}</div>
      <h2>Meu Perfil</h2>
      <p>Faça login para acessar<br>seus favoritos e preferências.</p>
    </div>
    ${buildNav('perfil')}
  `;
  lucide.createIcons();
...
function buildNav(active) {
  const items = [
    ...
    { nav: 'perfil', icon: 'user',    label: 'Perfil' },
  ];
  return `
